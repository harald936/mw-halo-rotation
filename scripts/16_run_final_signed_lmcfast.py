"""
SIGNED OMEGA RUN: Dynesty + Mock Streams + Fast LMC + sigma_sys
===============================================================
5 free parameters: v_h, r_h, q_z, Omega_p, sigma_sys
Omega_p prior: U(-0.5, 0.5)
200 mock particles per stream (spray method)
LMC perturbation on Orphan-Chenab only

This next-run script keeps the exact science model for the MW force and
the Orphan-Chenab perturbation, but accelerates the LMC back-integration
by using the underlying static halo density inside Chandrasekhar friction.
"""
...
from src.potential.lmc import build_lmc_potential_fast
...
def log_likelihood(theta):
    ...
    try:
        ...
        try:
            lmc_pot, _ = build_lmc_potential_fast(pot)
            pot_lmc = pot + [lmc_pot]
...
if __name__ == "__main__":
    ...
    print("SIGNED OMEGA RUN — fast LMC path")
    ...
    results_file = os.path.join(REPO, "results", "dynesty_final_signed_lmcfast.npz")
    ...
    print(f"\n{'=' * 55}")
    print("SIGNED OMEGA RESULTS — fast LMC path")
    print(f"{'=' * 55}")
    ...
    fig.savefig(os.path.join(PLOTS, "final_signed_lmcfast_corner.png"), dpi=150)
    ...
    ax.set_title(r"Figure Rotation Rate — Signed $\Omega_p$ (Fast LMC)")
    ax.legend(fontsize=9)
    plt.tight_layout()
    fig2.savefig(os.path.join(PLOTS, "final_signed_lmcfast_omega_p.png"), dpi=200)
